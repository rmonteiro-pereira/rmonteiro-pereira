...
| **[energy-forecast-drift](https://github.com/rmonteiro-pereira/energy-forecast-drift)** | The MLOps axis, measured on two years of **real PJM load from the EIA**: temporal-leakage control, walk-forward backtesting (**2.72% vs 11.73% MAPE** against seasonal-naive on identical folds, 24/24 horizons), four drift detectors (PSI/KS, target, prediction, performance) with a retrain-trigger verdict, gated promotion through an MLflow registry, and a daily cron that republishes every number by pull request — with a **[live dashboard](https://energy-forecast-drift.vercel.app)**. | Python · LightGBM · MLflow · FastAPI · React |
| **[vit-vertebrae-pose](https://github.com/rmonteiro-pereira/vit-vertebrae-pose)** | A controlled deep-learning benchmark rather than a training script: **169 archived runs** collapsed into a 9-model × 6-configuration grid of Vision Transformers for cervical-vertebra landmark localisation, **95% CI on every number**, each re-derivable from a single run hash — 243 tests, verifiable with no GPU and no dataset. | PyTorch · ViTPose · uv · pytest |
